*! Tim's modification of -mibeta- from SSC to allow it to save R2 results
*! version 1.0.0  07jan2010
capture program drop mibeta_tim
capture program drop _chk_saving
capture program drop _di_stats

program mibeta_tim, eclass
	version 11
	syntax [anything(everything name=rhs)] [aw fw iw pw]	  ///
				[, 	FISHERZ		 	  ///
					NOCOEF 	  		  ///
					MIOPTS(string asis)	  ///
					SAVER2				///		Option to allow saving R2 rather than adjusted R2
					*			  /// <regopts>
				]
	if ("`weight'"!="") {
	...
	// Inserted by Tim
	if "`saver2'" == "saver2" qui _di_stats r2_1, `original' sqrt
	// ***********************
