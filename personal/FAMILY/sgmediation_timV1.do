*! Modifying sgmediation by Phil Ender, to return a few more results
...
capture program drop sgmediation_tim
program define sgmediation_tim, rclass
...
`quietly' {
 ...
 `prefix' regress `varlist' `iv' `cv' if `touse', `options'			// The `options' added by Tim
 local ccoef=_b[`iv']
 local cse =_se[`iv']

 display
 display "Model with mediator regressed on iv (path a)"
 `prefix' regress `mv' `iv' `cv' if `touse', `options'			// The `options' added by Tim
 ...
 `prefix' regress `varlist' `mv' `iv' `cv' if `touse', `options'			// The `options' added by Tim
/************ Added by Tim ***********/
 local N = e(N)
/**********************************/
}
...
/************ Added by Tim ***********/
return scalar sobel_se = `serr'
local lci = `sobel' - invnormal(0.975) * `serr'
local uci = `sobel' + invnormal(0.975) * `serr'
return scalar sobel_lci = `lci'
return scalar sobel_uci = `uci'
return scalar sobel_z = `stest'
return scalar sobel_p = 2*(1-norm(abs(`stest')))
return scalar N = `N'
/*************************************/
