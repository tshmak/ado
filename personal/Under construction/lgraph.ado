*! version 3.0.2 Timothy Mak Nov2013.
...
syntax varlist(num min=2) [if] [in] [aweight pweight iweight fweight] [, Statistic(string) ERRortype(string) ///
	...
	addplot(string asis) SCHeme(passthru) MSYMbolrotate(string)  ///
	median Quantile(numlist min=2 max=2) minmax /* Obsolete options - included for backward compatibility 
	*/ NATurallegend LPATternrotate(string) /// Experimental options
	*]
...
if "`wide'" == "wide" {
	...
	lgraph _dummyvar `xvar' _j [`weight' `exp'], statistic(`statistic') errortype(`errortype') `legend' separate(`separate') ///
		loptions(`loptions') eoptions(`eoptions') leoptions(`leoptions') foptions(`foptions') `bw' fit(`fit') `plotorderoptions' ///
		colorgradient(`colorgradient') `marker' by(`by') alsocollapse(`alsocollapse') nopreserve addplot(`addplot') ///
		msymbolrotate(`msymbolrotate') lpatternrotate(`lpatternrotate') `scheme' `options'
...
qui collapse `alsocollapse' (`statistic') statistic=`depvar' `scale_collapse' (count) count=`depvar' /// 
	`extratocollapse' if `touse' [`weight' `exp'], by(`xvar1' `xvar2' `xvar3')
...
// Parse msymbolrotate
if `"`msymbolrotate'"' != "" {
	if `"`msymbolrotate'"' == "on" | real(`"`msymbolrotate'"') != . {
		local msymlist circle diamond square triangle x plus circle_hollow diamond_hollow square_hollow ///
			triangle_hollow smcircle smdiamond smsquare smtriangle smx
		if `"`msymbolrotate'"' == "on" local msymN 15 
		else local msymN = max(min(15, floor(`msymbolrotate')), 1)
	}
	else {
		local msymlist `msymbolrotate'
		local msymN : word count `msymbolrotate'
	}
}

// Parse lpatternrotate
if `"`lpatternrotate'"' != "" {
	if `"`lpatternrotate'"' == "on" | real(`"`lpatternrotate'"') != . {
		local lpatlist solid dash vshortdash longdash_dot longdash dash_dot dot shortdash_dot tight_dot dash_dot_dot ///
			longdash_shortdash dash_3dot longdash_dot_dot shortdash_dot_dot longdash_3dot
		if `"`lpatternrotate'"' == "on" local lpatN 15
		else local lpatN = max(min(15, floor(`lpatternrotate')), 1)
	}
	else {
		local lpatlist `lpatternrotate'
		local lpatN : word count `lpatternrotate'
	}
}
...
foreach level in `xvar2levels' { 
	...
	if "`xvar3'" != "" {
		...
		foreach level3 in `xvar3levels' {
			if `"`msymbolrotate'"' != "" {
				if `msym_j' > `msymN' local msym_j = 1
				local msym_touse : word `msym_j' of `msymlist'
				local msymoption msym(`msym_touse')
				local msym_j = `msym_j' + 1
			}

			if `"`lpatternrotate'"' != "" & "`fit'" == "" {
				if `lpat_j' > `lpatN' local lpat_j = 1
				local lpat_touse : word `lpat_j' of `lpatlist'
				local lpatoption lpat(`lpat_touse')
				local lpat_j = `lpat_j' + 1
			}
			...
			if r(N) > 0 {
				...
				local argument `argument' (connect statistic `xvar1' if `xvar2' == `level' & `xvar3' == `level3', `line_option' ///
					`msymoption' `lpatoption' `nomarker' `extraoptions') 
		...
		if `"`msymbolrotate'"' != "" {
			if `msym_i' > `msymN' local msym_i = 1
			local msym_touse : word `msym_i' of `msymlist'
			local msymoption msym(`msym_touse')
			local msym_i = `msym_i' + 1
		}

		if `"`lpatternrotate'"' != "" & "`fit'" == "" {
			if `lpat_i' > `lpatN' local lpat_i = 1
			local lpat_touse : word `lpat_i' of `lpatlist'
			local lpatoption lpat(`lpat_touse')
			local lpat_i = `lpat_i' + 1
		}

		local argument `argument' (connect statistic `xvar1' if `xvar2' == `level', `line_option' `msymoption' `lpatoption' `nomarker' `extraoptions') 
		local counter = `counter' + 1
	}
		...
		if "`xvar3'" != "" {
			...
			foreach level3 in `xvar3levels' {
				...
				if yesno[`i',`j'] == 1 {
					...
					local argument3 `argument3' (`anything' statistic `xvar1' [`aw'] if `xvar2' == `level' & `xvar3' == `level3', ///
						`line_option' `lpatoption' `extraoptions') 
					local counter = `counter' + 1
				}
				local j = `j' + 1
			}
			...
			local argument3 `argument3' (`anything' statistic `xvar1' [`aw'] if `xvar2' == `level', `line_option' `lpatoption' `extraoptions') 
			local counter = `counter' + 1
		}
...
if `"`legend'"' == "nolegend" | `max' == 1 { 
	...
	if "`xvar3'" != "" {
		...
		forvalues i = 1/`max' { 
			local labelled = 0
			forval j = 1/`nlevels3' {
				if `"`colorg_by'"' != "by" & `"`msymbolrotate'"' == "" & `"`lpatternrotate'"' == "" local extracond if `labelled' == 0
