...
{marker note}{...}
{title:Note}
{p 4 4 2}- In theory, all functions in {helpb mata} can be used, including user-defined ones. However, all variables in {cmd:matamatrix} must be Stata matrices. {helpb scalar}s and mata variables cannot be included.{break}
		- If speed is an issue, then {cmd:matamatrix} may not be the best idea.{break} 
		- {cmd:matamatrix} has not been tested on all {helpb mata} commands. Please let me know if you encounter any bugs while using it. 
