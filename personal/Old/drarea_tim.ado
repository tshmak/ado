...
/*
2/4/07 version 1.02 allows the user to specify the overlap colour as the third color in the color() option
2/4/07 version 1.03 allows the overlapping areas to be saved into the dataset but to get 3 overlapping areas doesn't work so isn't a documented feature.
*/
 
program def drarea_tim
version 9.2
syntax varlist(min=5 max=5) [if] [in] [, Color(string asis) Add Generate(string) *]
...
twoway (rarea `h1' `l1' `x' if `zone'~=., fc("`c1'") lc(black) lw(0) ) (rarea `h2' `l2' `x' if `zone'~=., fc("`c2'")  lc(black) lw(0) ) ////
 (rarea `h4' `l4' `x', fc("`c3'") lc(black) lw(none) ) `xtra',  scheme(s2color) legend(order(1 2)) `xoptions'
*/

/* Tim modified */
twoway (rarea `h1' `l1' `x' if `zone'~=., col("`c1'") lw(0) ) (rarea `h2' `l2' `x' if `zone'~=., col("`c2'")  lw(0) ) ////
 (rarea `h4' `l4' `x', col("`c3'") lw(none) ) `xtra',  scheme(s2color) legend(order(1 2)) `xoptions'
